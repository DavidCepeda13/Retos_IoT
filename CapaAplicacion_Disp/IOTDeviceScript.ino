...
// Pin del LED integrado del ESP8266 (GPIO2 = D4 en NodeMCU)
#define LED_PIN LED_BUILTIN
// Cantidad de parpadeos del LED cuando se recibe un evento
#define LED_BLINK_COUNT 10
// Intervalo de parpadeo del LED en milisegundos
...
const char ssid[] = "DavC"; // TODO cambiar por el nombre de la red WiFi
// Contraseña de la red WiFi
const char pass[] = "313159255027"; // TODO cambiar por la contraseña de la red WiFi
...
const char MQTT_HOST[] = "3.80.206.10"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
...
float humi;
// Indica si hay un evento LED activo
bool ledEventActive = false;
// Cantidad de parpadeos restantes
int ledBlinksRemaining = 0;
// Tiempo del último cambio de estado del LED
long long int ledLastToggle = 0;
// Estado actual del LED (encendido/apagado)
bool ledState = false;
// Mensaje del evento para mostrar en pantalla
String eventMessage = "";
...
void receivedCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (data.indexOf("EVENT") >= 0) {
    // Evento de procesamiento: activar parpadeo del LED
    Serial.println("\n>>> Evento recibido: " + data);
    ledEventActive = true;
    ledBlinksRemaining = LED_BLINK_COUNT * 2; // cada parpadeo = encender + apagar
    ledLastToggle = millis();
    ledState = false;
    eventMessage = data;
    // También mostrarlo como alerta en pantalla
    alert = data;
    alertTime = millis();
...
/**
 * Procesa el parpadeo del LED cuando hay un evento activo.
 * Alterna el LED encendido/apagado según LED_BLINK_INTERVAL.
 */
void processLedEvent() {
  if (!ledEventActive) return;

  if ((millis() - ledLastToggle) >= LED_BLINK_INTERVAL) {
    ledLastToggle = millis();
    ledState = !ledState;
    digitalWrite(LED_PIN, ledState ? LOW : HIGH); // LED_BUILTIN es activo en LOW
    ledBlinksRemaining--;

    if (ledBlinksRemaining <= 0) {
      ledEventActive = false;
      digitalWrite(LED_PIN, HIGH); // Apagar LED
      Serial.println(">>> Evento LED finalizado");
    }
  }
}
...
void setup() {
  ...
  // Configurar el LED integrado como salida y apagarlo
  pinMode(LED_PIN, OUTPUT);
  digitalWrite(LED_PIN, HIGH); // HIGH = apagado en LED_BUILTIN
...
void loop() {
  ...
  // Procesar parpadeo del LED si hay evento activo
  processLedEvent();
